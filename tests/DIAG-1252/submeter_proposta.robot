...
Submeter política importando CSV 
    ...
    SubmissionPage.E informar a politica    POLITICA_DAVO    PROP_CPF
    Quando fizer o upload do arquivo    Entradas.csv
    E acionado o botão submeter proposta
    SubmissionPage.Então é exibido um toast com a mensagem    finalizada com sucesso!

Validar submissão somente com arquivo CSV
    [Tags]    DIAG-1254
    DetalhamentoPage.Dado ter selecionado um periodo para consultar
    E acionado o botão submeter proposta
    SubmissionPage.E informar a politica   BATCH_TST    PROP_CPF    
    Quando fizer o upload do arquivo    teste.pdf
    SubmissionPage.Então é exibido um toast com a mensagem    Extensão de arquivo não suportada! Somente arquivos CSV podem ser importados!
...
Submissão de política com arquivo importado vazio
    ...
    SubmissionPage.Então é exibido um toast com a mensagem    O arquivo está vazio!
